  ...
  return (
    <div className="page-container edu-page mcq-page">
      ...
      <div className="edu-page-actions">
        <button type="button" className="edu-page-action" onClick={() => setIsExpandedView((value) => !value)}>
          {isExpandedView ? t('vocabulary.compact') : t('vocabulary.detailed')}
        </button>
        ...
        <button type="button" className="edu-page-action" disabled={displayItems.length === 0} onClick={() => openPrintWindow(generatePrintExamSheet({ questions: displayItems, title: t('edu.multiple-choice-questions.title'), language, ...printOptions }))}>
          {t('vocabulary.print')}
        </button>
      </div>

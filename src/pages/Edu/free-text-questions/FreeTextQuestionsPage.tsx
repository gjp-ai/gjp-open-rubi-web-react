...
export const EduFreeTextQuestionsPage = () => {
  const kind = 'free-text-questions' as EduQuestionKind
  ...
  return (
    <div className="page-container edu-page free-text-page">
      ...
      <div className="edu-page-actions">
        ...
        <button type="button" className="edu-page-action" disabled={displayItems.length === 0} onClick={() => openPrintWindow(generatePrintExamSheet({ questions: displayItems, title: t('edu.free-text-questions.title'), language, ...printOptions }))}>{t('vocabulary.print')}</button>
      </div>

      {loading ? (
        <div className="free-text-list" aria-hidden>{skeletonItems.map((item) => <div key={item} className="free-text-card skeleton" />)}</div>
      ) : error ? (
        <div className="state-card state-card--error">{error}</div>
      ) : (
        <div className="free-text-list">
          {displayItems.map((question) => (
            <FreeTextQuestionCard key={question.id} question={question} isExpandedView={isExpandedView} lang={language} />
          ))}
        </div>
      )}
...
export default EduFreeTextQuestionsPage

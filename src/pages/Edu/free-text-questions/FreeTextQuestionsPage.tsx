...
export const EduFreeTextQuestionsPage = () => {
  ...
  const handlePrintExam = () => {
    const htmlContent = generatePrintExamSheet({
      ...
      title: t('edu.free-text-questions.title'),
      language,
  ...
  return (
    <div className="page-container edu-page free-text-page">
      ...
      {/* ───── Content ───── */}
      {loading ? (
        <div className="ftq-grid" aria-hidden>{skeletonItems.map((item) => <div key={item} className="ftq-card skeleton" />)}</div>
      ) : error ? (
        <div className="state-card state-card--error">{error}</div>
      ) : displayItems.length === 0 ? (
        <div className="ftq-empty">📭 {t('edu.no_results')}</div>
      ) : (
